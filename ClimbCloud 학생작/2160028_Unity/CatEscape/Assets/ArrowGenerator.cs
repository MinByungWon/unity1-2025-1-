/*
 **화살 오브젝트를 1초에 한개씩 생성하는 알고리즘
 * - Update 메서드는 프레임마다 실행되고 앞 프레임과 현재 프레임 사이의 시간 차이는 Time.detaTime에 대입
 ...
 * - Random.Range 메서드 : 랜던 값을 쉽게 생성할 수 있는 방법
 *  -- Random 클래스는 흔히 요구되는 다양한 랜덤 값을 쉽게 생성할 수 있는 방법을 제공
...
public class ArrowGenerator : MonoBehaviour
{
     ...
     *  - 멤버변수 선언시 public으로 선언하면 inspector 창에서 Prefab 설계도 대입할 수 있도록 보임
     *  - 화살 대량 생산을 위해서 양산기계(제너레이터스크립트)에 넘겨 줄 prefab 설계도를 넘겨 주어야 함
     */
    ...
    void Update()
    {
        ...
        if(fDeltaTime > fArrowCreateSpan)
        {
             ...
             * - RPG 게임이라면 수많은 아이템, 캐릭터, 배경 등 모든 것들을 어떻게 미리 만들어놓을 수 있을까?
             *      그러므로 게임오브젝트의 복제본을 생성
             * - Instantiate(GameObject original, Vector3 position, Quaternion rotation)
             *  -- GameObject original : 생성하고자 하는 게임 오브젝트명, 현재 씬에 있는 게임 오브젝트나 Prefab으로 선언된 객체를 의미함
             *  -- Vector3 position : Vector3으로 생성될 위치를 설정함
             *  --Quaternion rotatio : 생성될 게임오브젝트의 회전값을 저장
             */
