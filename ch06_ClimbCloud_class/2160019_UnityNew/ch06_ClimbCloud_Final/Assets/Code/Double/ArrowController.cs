...
public class DiaController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (fPlayerArrowDir1 < fArrowRadius + fPlayerRadius)
        {
            ...
            director1.GetComponent<GameDirector>().fDcreasePointsArrow();     //GameDirector에 있는 요소 중 하나인 fCreasePointApple을 불러와서 값을 올림

            //먹고 파괴
            Destroy(gameObject);

            // Bcat이 화살에 맞으면 knockBack되게 만듬
            m_Cat.GetComponent<YCatController>().YNnockBack();


        }

        if (fPlayerArrowDir2 < fArrowRadius + fPlayerRadius)
        {
            GameObject director2 = GameObject.Find("GameDirector");
            director2.GetComponent<GameDirector>().fDcreasePointsArrow();     //GameDirector에 있는 요소 중 하나인 fCreasePointApple을 불러와서 값을 올림

            //먹고 파괴
            Destroy(gameObject);

            // Bcat이 화살에 맞으면 knockBack되게 만듬
            m_Bcat.GetComponent<BCatController>().BNnockBack();
